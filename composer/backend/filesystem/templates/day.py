...
class DayTemplate(Template):
    ...
    def build(self):
        (date, day, month, year) = (self.next_day.day, self.next_day.strftime('%A'), self.next_day.strftime('%B'), self.next_day.year)
        ...
        scheduled, tasklistfile = scheduling.get_scheduled_tasks(tasklistfile, self.next_day)
        tomorrow, tasklistfile = self._get_tasks_for_tomorrow(tasklistfile)
        # TODO: do this mutation elsewhere
        self.planner.tasklistfile = tasklistfile  # update the tasklist file to the post-processed version
        self.agenda = ''
        if scheduled:
